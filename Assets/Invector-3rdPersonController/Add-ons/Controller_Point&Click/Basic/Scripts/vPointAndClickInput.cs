...
namespace Invector.vCharacterController.PointClick
{
    ...
    public class vPointAndClickInput : vThirdPersonInput
    {
        ...
        {
            if (GameManager.s_instance.mouseOnUI)
                return;
        ...
        {
            if (!NearPoint(cursorPoint, transform.position) && target ) // && Original condition
               // ((gameObject.tag == "Player" && target.tag == "Land") || (gameObject.tag == "PlayerShip" && target.tag == "Ocean")))
            {
